...
shinyServer(
  function(input, output) {
    ...
    output$plots <- renderPlot({
      ...
      points(input$hp, pred_mpg(), col='blue', cex=3)                 
      # (1, 2)
      with(mtcars, plot(cyl, mpg,
                        xlab='Np of cylinders',
                        ylab='Miles Per Gallon (MPG)',
                        main='MPG by cylinders'))
      points(as.numeric(input$cyl), pred_mpg(), col='blue', cex=3)  
      # (1, 3)
      with(mtcars, plot(wt, mpg,
                        xlab='Weight (lb/1000)',
                        ylab='Miles Per Gallon (MPG)',
                        main='MPG by weight'))
      points(adj_weight(), pred_mpg(), col='blue', cex=3)  
    })
  }
)